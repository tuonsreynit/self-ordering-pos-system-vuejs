    ...
    <div class="w-[80%] max-w-[450px] bg-white rounded overflow-hidden shadow-sm">
        <div class="p-5">
            <div class="flex items-center justify-between pb-2.5 border-b-[#ccc] border-b">
                <div class="text-xl font-semibold">បញ្ចូលទំនិញថ្មី</div>
            ...
            <div>
                <div class="py-2.5 flex flex-col items-start text-start">
                    ...
                    <div class="flex justify-between w-full border-b mb-4 border-b-gray-400  p-1.5">
                        ...
                        <select name="categoryId" v-model="categoryId" class="focus:outline-none w-[200px]" required>
                            <option value="" disabled>ប្រភេទទំនិញ</option>
                            <option v-for="category in categories" :value="category._id" class="">
                                {{ category.name }}
                            </option>
                        </select>
                    </div>
                    <input type="text" v-model="name" placeholder="បញ្ចូលឈ្មោះទំនិញ" required class="w-full mb-4 p-1.5 border-b border-b-gray-400 outline-none" />
                ...
                <div class="flex items-center justify-end pt-2.5">
                    <button @click="submitProduct" class="ml-10 rounded bg-[#1C355D] hover:bg-[#2691d9] text-white px-3 py-1.5" type="submit">រួចរាល់</button>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        submitProduct() {
            ...
            axios.post(this.apiUrl+'/product/create', formData, {
                headers: {
