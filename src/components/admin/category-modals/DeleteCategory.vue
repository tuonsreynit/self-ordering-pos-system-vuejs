        ...
        <div class="p-5">
            ...
            <div>
                <div class="py-2.5 flex items-start text-start m-4 text-gray-500">
                    <span>តើអ្នកពិតជាចង់លុបប្រភេទទំនិញ​ឈ្មោះ</span> <span class="text-black">{{ category.name }}</span> នេះមែនឬ?
                </div>
                <div class="flex items-center justify-end pt-2.5 border-t border-t-[#ccc]">
                    ...
                    <button @click="deleteCategory()" class=" ml-4 rounded bg-[#fc1a1a] hover:bg-[#d12439] text-white px-3 py-1.5" type="submit">លុប</button>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        deleteCategory() {
            this.isLoading = true
            axios.delete(this.apiUrl+'/category/'+this.category._id, 
            {
                headers: {
