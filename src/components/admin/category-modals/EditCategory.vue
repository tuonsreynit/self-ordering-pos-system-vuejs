    ...
    <div class="w-[80%] max-w-[450px] bg-white rounded overflow-hidden shadow-sm">
        <div class="p-5">
            <div class="flex items-center justify-between pb-2.5 border-b-[#ccc] border-b">
                <div class="text-xl font-semibold">កែប្រភេទទំនិញ</div>
            ...
            <div>
                <div class="py-2.5 flex flex-col items-start text-start">
                    <div>
                        <div id="profile" @click="openFileInput">
                            ...
                            <label for="file" :style="{ backgroundImage: `url(${apiUrl}/static/get${category.image.substring(1)})`, backgroundRepeat: 'no-repeat', backgroundSize: 'cover' }" class="clicktext text-white rounded-full p-1.5" v-if="!image"><div class="text-white bg-gray-600/50 p-2">ចុចនៅទីនេះដើម្បីបញ្ចូលរូបភាព</div></label>
                    ...
                    <input type="text" v-model="name" placeholder="បញ្ចូលឈ្មោះប្រភេទទំនិញថ្មី" required class="w-full mb-4 p-1.5 border-b border-b-gray-400 outline-none" />
                ...
                <div class="flex items-center justify-end pt-2.5">
                    <button @click="updateCategory" class="ml-10 rounded bg-[#1C355D] hover:bg-[#2691d9] text-white px-3 py-1.5" type="submit">រួចរាល់</button>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        updateCategory() {
            ...
            formData.append('_id', this.category._id)
            formData.append('name', this.name)
            formData.append('description', this.description)
            formData.append('image', this.selectedFile)
            axios.put(this.apiUrl+'/category', formData,
            {
                headers: {
