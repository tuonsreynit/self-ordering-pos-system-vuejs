        ...
        <div class="w-[80%] mix-w-[450px] max-w-[570px] bg-white rounded overflow-hidden shadow-sm">
            <div class="p-5">
                ...
                <div class="py-2.5 flex items-start text-start m-4 text-gray-500">
                    តើអ្នកពិតជាចង់អ្នកប្រើប្រាស់​ឈ្មោះ <span class="text-black">{{ user.username }}</span> នេះមែនឬ?
                </div>
                <div class="flex items-center justify-end pt-2.5 border-t border-t-[#ccc]">
                    ...
                    <button @click="deleteUser" class=" ml-4 rounded bg-[#fc1a1a] hover:bg-[#d12439] text-white px-3 py-1.5" type="submit">Delete</button>
                </div>
            </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        deleteUser() {
            this.isLoading = true
            axios.delete(this.apiUrl+'/user/'+this.user._id, {
                headers: {
