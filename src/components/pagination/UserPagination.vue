...
<div>
    <ul v-if="pages" class="list-reset flex">
        <li class="inline-block hover:bg-blue-lightest">
            <router-link :to="{ name: 'user', query: { page: currentPage - 1 } }" class="no-underline block" :class="{
        ...
        <li v-for="(page, index) in range" :key="index" class="inline-block hover:bg-blue-lightest" :class="{ '': index == range.length - 1 }">
            <router-link v-if="page != '...'" :to="{ name: 'user', query: { page: page } }" class="no-underline block py-1 px-1" :class="{'text-[#4280E3]': page == currentPage }" tag="a">
